...
| Verify symbols outside `sys/` (e.g. `stand/`) | `chapters.yaml` `extra_search_dirs:` | Per-chapter list of additional grep roots, joined to `~/freebsd-src`. `_resolve_search_roots` always includes `<src>/sys` and appends each existing extra. The cache key embeds a sorted-tuple suffix so widening roots for one chapter does NOT poison the sys-only cache for others. Use this when a chapter's subject (boot loader, userland tool) lives outside `sys/`. **`source_dirs` is a different key and does NOT widen the verifier** — it steers the writer. Adding a non-`sys/` dir to `source_dirs` alone invites the writer to cite symbols the fact-checker cannot see, which then come back as fabricated "missing structs" and cost the chapter its Accuracy pass. That exact mistake was made on ch4 (2026-09-01, fixed 2026-09-04): `usr.sbin/config` was added to `source_dirs` with a comment claiming the fact-checker could now see `struct cfgfile`. Whenever you add a non-`sys/` `source_dirs` entry, add it here too. |
| Diagnose a chapter that stops with no output | Nothing — it is automatic | `install_hang_detector()` runs from `main()`. When the main thread stops calling `beat()` for `DAEMONDOCS_HANG_DUMP_SEC` (default 1800s, `0` disables), a daemon thread prints the main thread's stack to the chapter log and keeps going. It never kills anything. Wrap a new long phase in `with heartbeat("label"):` so the dump names it, and call `beat()` inside long loops. Works even when stuck in a C-level regex, which a Python-level timeout cannot interrupt. **Before dumping it asks the endpoint whether the model is actually decoding** — see the note below. Startup prints which mode is active; if it says "WITHOUT endpoint liveness", restart llama-server with `--metrics`. **Liveness is a RATE, not a boolean** (`_DECODE_MIN_RATE`, env `DAEMONDOCS_DECODE_MIN_RATE`, default 1.0 decode/s): before 2026-09-01 the probe returned `second > first`, so a 0.1/s trickle counted as a busy model and suppressed the dump for ch7's entire 41-minute wedge. If you touch `_endpoint_is_decoding`, keep the fail-safe direction (unavailable / flat / reset / trickle all → "not decoding") and re-run `tests/test_regex_hang.py` group 4. |
...
- **`_verify_structs`' shape filter must match all three definition
  ...
  grep. Leading whitespace and the `typedef`/`extern` prefixes are
  allowed **only on the brace alternative**; the K&R alternative
  (`struct foo` at end of line) keeps `^` because an indented
  `struct thread *td` parameter would otherwise verify as a
  definition. `extern struct NAME {` was the fifth spelling added
  (2026-09-04, 2 in the tree: `bio_ops` in `sys/sys/buf.h`,
  `ar8327_led_mapping`) after it cost ch11 its Accuracy pass on both
  arms of the 2026-09-03 A/B — `pattern_template` matched the line and
  only this stage-2 filter dropped it. The required trailing brace is
  what makes widening the prefix safe: `extern struct NAME;` and
  `extern struct NAME *p;` are declarations, not definitions.
