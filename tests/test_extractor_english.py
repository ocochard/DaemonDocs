...
# Frozen copy, not the live artifact: this file is generated by a
# chapter, so reading it directly couples this suite to whenever that
# chapter last regenerated. See tests/fixtures/README.md.
ch40 = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                    "fixtures", "usb_README.snapshot.md")
if os.path.exists(ch40):
    ...
    #
    # `xhci_softc` and `router_softc` were in this list until 2026-09-02
    # and were removed as WRONG EXPECTATIONS, not as a concession to a
    # failing suite. Both are real structs (`xhci_softc` in
    # sys/dev/usb/controller/xhci_pci.c, `router_softc` in
    # sys/dev/thunderbolt/router_var.h) and the chapter still covers both
    # topics heavily (19 xHCI mentions, 16 router/Thunderbolt). But
    # neither name appears in the chapter in its committed form either --
    # `git show HEAD:sys/dev/usb/README.md` has zero occurrences of each,
    # the same as the working tree. The assertion therefore pinned text
    # the pipeline has never reliably produced; the chapter discusses
    # those subsystems in prose without naming the softc types. Keeping
    # the check would have meant treating a stable, correct chapter as
    # permanently broken.
    for good in ("usb_host_endpoint", "usb_xfer", "nhi_cmd_frame"):
    ...
    check("fixture usb_README.snapshot.md present", False, f"missing: {ch40}")
