...
FreeBSD does not run transfer-completion callbacks inside the interrupt handler. Instead each USB bus owns a small family of kernel threads — an explore process for enumeration, a control-transfer process, and separate callback processes for Giant-locked, bulk, and isochronous completions. The interrupt handler only enqueues a message; the thread does the work that may sleep. That is what lets a driver submit a control transfer with `usbd_transfer()` and then sleep on a condition variable until it completes, without stalling the whole bus.
